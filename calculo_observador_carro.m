...
% 3. Análisis de Observabilidad de Kalman
O_mat = obsv(A, C);
rango_O = rank(O_mat);

disp('--- PARTE 1: MATRICES Y OBSERVABILIDAD ---');
disp('Matriz A:');
disp(A);
fprintf('El rango de la matriz de observabilidad es: %d\n', rango_O);

if rango_O == 4
    disp('¡ÉXITO! El sistema es completamente observable (Rango completo).');
else
    disp('ERROR: El sistema NO es observable. Revisar matrices.');
end
